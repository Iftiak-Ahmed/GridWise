...
- solar_reduction: usable solar is reduced during specific hours.
  ...
  Spelled-out fractions work the same way: "cut in half" / "half the normal output" -> 0.5
  remaining (factor = 0.5 if phrased as what remains, or 0.5 if phrased as a 50% cut, since
  both give the same remaining fraction here); "down to a quarter" -> factor = 0.25; "leaves
  three-quarters" -> factor = 0.75. Always resolve to the single remaining-fraction number.
...
- "noon" = hour 12. "midnight" = hour 0. "noon to 2 PM" -> [12,13]. "10 PM until midnight" \
  -> [22,23].
- "from X o'clock" or spelled-out hours without AM/PM (e.g. "from one until three", \
  "washing from one until three will leave roughly one-fifth of normal solar output"): infer \
  from context (campus daytime activity, other hours in the note, typical operating hours) \
  which of the two 12-hour readings is meant; if truly ambiguous, still pick the single most \
  plausible reading rather than refusing.
...
def _call_groq(operator_notes: list[str], battery_capacity_kwh: float) -> list[dict[str, Any]]:
    ...
    # No self-repair retry here: Gemini is a full independent second real-LLM attempt right
    # after this, so retrying Groq itself would only add latency against the 30s judge budget
    # for a case Gemini already covers.
